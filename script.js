...
function processAndShow() {
    ...
    const map = [
        ...
        {ar:'in-height', he:'in-weight', out:'row-measurements'}
    ];
    ...
    map.forEach(f => {
        ...
        if(row) {
            ...
            const he = row.querySelector('.he');
            if(ar) ar.innerText = document.getElementById(f.ar).value;
            if(he) he.innerText = document.getElementById(f.he).value;
        }
    });
...
// دالة لتطبيق تأثير الختم على الصورة
function applyStampEffect() {
    ...
    // رسم الختم الدائري
    const stampRadius = Math.min(canvas.width, canvas.height) * 0.2;
    const stampX = canvas.width - stampRadius - 30;
    const stampY = canvas.height - stampRadius - 30;
    ...
    ctx.font = 'bold ' + Math.floor(stampRadius * 0.5) + 'px Arial';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('default-photo.jpg')
        ...
        .then(blob => {
            ...
            reader.onload = (e) => {
                ...
                photoImg.onload = () => applyStampEffect();
            };
            reader.readAsDataURL(blob);
        })
        .catch(err => console.log('صورة افتراضية غير متوفرة'));
    ...
    document.getElementById('in-photo').onchange = (e) => {
        ...
        r.onload = (ev) => {
            ...
            // إضافة تأثير الختم بعد تحميل الصورة
            photoImg.onload = () => applyStampEffect();
        };
